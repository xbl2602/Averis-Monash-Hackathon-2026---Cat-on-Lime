...
- **线上实测校准（同日，推翻上面的 10s/12s/20s）**：部署后用无标签的 SI/BL 强制走抽取兜底，实测**线上 Gemini 做字段抽取要 8~10s、偶尔 16s**
  （分类只要约 2s）。10s 单次超时把"慢但能用"的 Gemini 抽取当成卡住切断了：显式指定 Gemini 时直接退回只有规则的结果（字段全空）。
  改为：单次尝试上限回到 20s（显式指定、没有备用模型时值得多等）；回退链里单个模型最多 15s、整条链 22s（首选卡满 15s 后 DeepSeek 实测约 3s，还剩 7s 够用）；
  Jev 维持 10s（实测约 1.5s）。真正治"卡住拖 42s"的是"超时不原地重试 + 关掉 SDK 自带重试"，这两条不变。
  复测：持续 500 → 2 次请求 2.1s；显式指定卡住 → 20s 放弃；首选卡住 → 15s 换下一个拿到结果；两个都卡住 → 22s 停；
  分类在"规则判不了 + Jev 失败 + 全部模型卡住"时 23s 给出尽力兜底，仍在 30s 上限内。
