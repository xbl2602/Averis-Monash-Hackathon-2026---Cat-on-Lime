...
### 2.10 ⚠️ 开发者模式：数据库重置/恢复（后端+GUI 均已实现：`/features/devmode`，2026-09-21）

**这个页面不是产品功能，是给团队/评委在验证阶段用的运维工具**（清空数据库、恢复成官方样例
状态）。接口契约见 [`SHARED_INTERFACES.md`](SHARED_INTERFACES.md)「开发者模式」一节。这一轮由
操作者特批越界实现了 GUI（`app/features/devmode/ui/`），按当初写好的硬性要求落地：

- **页面顶部持续可见的红色警示条**（`DevModeBanner`，`role="alert"`，SSR 就直接渲染出来，不是
  弹一次就消失的 toast），文案包含"开发者模式 / 不是正式产品功能 / 仅供内部与评委验证使用"。
- **没有放进主导航**（`app/dashboard/_components/nav-items.ts` 没加条目）——只在设置页最底部
  留了一行不显眼的小字链接（`app/dashboard/settings/page.tsx`）。
- **每个破坏性按钮都要求输入框手动打字确认**（`DangerAction` 组件）：清空按钮要打对
  `WIPE ALL DATA`，恢复按钮要打对 `RESTORE SAMPLE DATA`，逐字匹配前按钮是禁用状态——不是
  "确认吗？[是/否]"的一键弹窗。
- 口令走现有 `useAdmin()`/`adminRequest`，跟其它写接口同一套（不进 localStorage）。
- 页面展示 `GET /features/devmode/api` 的实时行数，每次操作后自动刷新。
- 如果以后要重新设计这个页面的视觉风格，随便改——警示条的文案/持续可见性、打字确认这两条
  硬性要求不能删，其余排版随意。
