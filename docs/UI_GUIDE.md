...
GUI 层面的**硬性要求**（操作者原话："你需要特别明显注明...不只是文档，包括GUI"）：

- **页面顶部要有持续可见的警示条**（不是弹一次就消失的 toast），文案至少包含"开发者模式 /
  仅供内部与评委验证使用 / 不是正式产品功能 / 操作不可逆"这几层意思，建议用红色/警示配色，
  和页面其余部分明显区分开，任何时候打开这个页面都要立刻看得到，不能翻到某个角落才发现。
- 这个页面本身建议不放进主导航（不要和 verification/sandbox 这些正式功能并列展示），可以单独
  一个不显眼的入口（比如设置页底部一行小字链接），或者干脆只在文档里给团队/评委分享直接链接。
- **每个破坏性按钮都要求输入框二次确认**：清空按钮要求用户在弹窗里手动打字输入
  `WIPE ALL DATA`（逐字，区分大小写），恢复按钮要求输入 `RESTORE SAMPLE DATA`——这两个短语后端
  会在请求体里逐字核对，不匹配直接 400，前端**不能用"确认吗？[是/否]"这种一键点掉的弹窗**代替
  输入确认，那样起不到防手滑的作用。
