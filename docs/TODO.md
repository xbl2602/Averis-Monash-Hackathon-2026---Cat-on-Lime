...
### [x] 提交日夜间排查（/goal）——详见 `docs/BUG_HUNT.md`、决策36
- **已修 9 条**，其中 3 条严重：提交文件混入 2768 条测试数据（3288→520）、人工覆盖能悄悄改掉提交答案且看不见、
  附件名无 `_SI/_BL` 时读不出/类型不对的附件被静默判 OK。另有运行状态切页丢失、SI/BL 对照表读串、
  Word/Excel 单元格内换行被吃掉、KPI 数字对不上、引擎版本号没跟着 +1、解锁口令探测方式。
- **待操作者决定**：[x] ~~email_004 那条人工覆盖要不要撤销~~（已解决：操作者说明是"更正 → 重跑"，重跑规则改成
  "后做的动作说了算"并按新规则重做，见决策37、`0a8daa2`；批量运行要不要也能重置人工结论仍待操作者决定）；
  [ ] GOAL.md C1 第一条校验命令改指向 `app/_components/providers.tsx`；[ ] 手动点一次"运行→切页→切回"确认。
- **没修**：扰动数据生成器的正则不接受中文标签（`scripts/perturb-generate.mjs:116`），导致 6 封测试数据本身缺重量行。
