...
- 单次尝试超时上限 20s（显式指定、没有备用模型时用得上；按实测校准：Gemini 字段抽取常见 8~10s）；只对"很快就失败"的暂时性错误（429 / 5xx / 网络）重试一次，**超时不重试**（直接换下一个）；
  SDK 自带的重试已关掉（`maxRetries: 0`），重试只发生在 `callLLM` 这一处。
- 回退链默认总时限 22s（`CHAIN_BUDGET_MS`），单个模型最多占 15s，剩余不足 3s 就不再尝试下一个；混合分类会把 Jev 已用掉的时间扣掉。
