...
5. **调用约定**：`callLLM` / `callJev` 统一 20s 超时、错误归一化（缺 key → 可读配置错误；上游故障 → 稳定 code，不透传原文）；所有结果写 `llm_call_cache`（键 = 用途 + 引擎版本 + provider + 模型 + 发送内容），输入不变不重复调用。**重试**（2026-09-21 P1-2）：`callLLM` 对超时/限流/上游5xx/网络错误在同一 provider 内自动重试一次（等 2 秒），401/403/400/422 等确定性错误不重试；`callJev` 不重试，失败直接交给上层混合引擎转下一级（见决策25）。这层同 provider 内重试与 `lib/shared/llm-chain.ts` 的跨 provider 降级链是叠加关系，不是互相替代。
