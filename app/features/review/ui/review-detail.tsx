...
export function ReviewDetail({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Always unfolded, in every module: a reviewer has to be able to read the source to judge it (operator's call,
          2026-09-22). Keyed by email so each item opens fresh even if the previous one was folded away. */}
      {rows.loading && !row && !item.body ? (
        <p className="text-sm text-fg-faint">Loading the email…</p>
      ) : (
        <EmailMessage key={item.email_id} body={item.body ?? row?.body} from={item.from ?? row?.from} subject={item.subject} defaultOpen />
      )}
