...
| **9/17 18:00 前** | **报名截止** |
| 现在 – 9/17 | 赛前准备（见第4节） |
| 9/18 | 题目公布，比赛开始，当天做方案+拆模块 |
| 9/18 – 9/21 | 开发冲刺（约4天build window） |
| 9/21 | 功能冻结，开始整合、debug、准备演示素材 |
| 9/22 上午 | 录视频、写slide、最终检查 |
| **9/22 12:00pm** | **初赛提交截止（Google Form）** |
| 待定 | 初赛选出前10名队伍，进入决赛：现场10分钟路演 + 5分钟Q&A（具体日期官方未公布，留意公告） |
...
团队零编程经验 + 时间紧，建议选**最省运维、AI最容易生成对的代码**的组合，而不是最"专业"的组合。这个组合也在调研中被验证为2026年AI编程工具圈公认的"低幻觉率"标准搭配（因为文档/训练数据里这个组合最常见，AI写起来出错最少）：

- 前端 + 部署：**Next.js**，部署到 **Vercel**（免费、git push 即部署、不用碰云控制台、AI 生成的 Next.js 代码质量普遍最高）
- 后端/数据：**Supabase**（托管 Postgres 数据库 + 鉴权 + 存储，不用自己搭服务器，清楚地满足"云基础设施"要求）
- AI 能力：**Claude API**（通过 Vercel AI SDK 调用），作为产品核心功能的一部分（不是"用AI写了代码"，是产品本身要用到AI）——评委看得到 AI 和云是两个独立、可指认的组件，而不是含糊带过

如果题目对技术栈有强约束（比如要做移动端、要处理大数据），9/18 当天再调整，但默认组合能覆盖大多数常见赛题方向，尤其是第2.1节推测的"企业后台/供应链"类题目。
