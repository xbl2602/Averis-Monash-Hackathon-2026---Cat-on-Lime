...
3. 每人认领一个模块（对应一个文件夹），**明确谁负责哪个文件夹**，写进 Discord 置顶或者一个共享文档里
4. 参考角色划分示例（可按各自强项调整，每个角色对应一个 feature 文件夹）：
   - **A：前端/UI 模块** — 页面结构、交互、演示时给评委看的界面
   - **B：数据与鉴权模块** — Supabase 数据结构、登录、云端配置
   - **C：核心AI功能模块** — 调用 LLM API 的那一个"评委30秒能看懂"的关键功能，外加准备demo用的备用/缓存数据（防止台上live API挂掉）
5. 如果某个功能必须用到别人模块的东西，先在 `SHARED_INTERFACES.md` 里写清楚"我需要什么格式的数据/接口"，而不是让自己的 AI 直接跑去改/读别人文件夹里的实现细节
...
- **功能模块拆分**：按官方给的三个步骤拆成三个 feature（已写进 [CLAUDE.md](CLAUDE.md)）：
  - [ ] `classification`（邮件分类）—— 认领人：______
  - [ ] `extraction`（字段抽取）—— 认领人：______
  - [ ] `comparison`（比对+人工确认触发）—— 认领人：______
